...
#/usr/local/bin/gunicorn zenofcoder.wsgi:application -w 2 -b 0.0.0.0:8000 --reload --access-logfile /var/log/gunicorn_access.log --error-logfile /var/log/gunicorn_error.log --log-level debug
/usr/local/bin/gunicorn zenofcoder.wsgi:application -w 2 -b 0.0.0.0:8000 --reload